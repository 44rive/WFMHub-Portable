...
1. Read the visible dashboard/table, `DEFINITIONS`, and `HELP` when present.
   ...
   is required. Do not assume sheets that are not present.
...
8. Attendance is observed from schedule plus Agent Status, with LILO as fallback
   and control. PTO/Away takes precedence. Attendance Review shows residual
   evidence only. Verint Activities are not observed attendance; they are the
   separate final post-day absence/shrinkage ledger.

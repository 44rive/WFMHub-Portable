...
function Set-SelectorNames {
    ...
    $groups = [ordered]@{
        "PCS_TL_ACTIVE" = @($teamGroup, "C", "D")
        "PCS_AGENT_ACTIVE" = @($agentGroup, "E", "F")
    }
    foreach ($name in @("PCS_PERIOD_LIST", "PCS_LOB_LIST")) {
        ...
        $created = $script:Workbook.Names.Add($name, $formula)
...
function Assert-SelectorIntegrity {
    ...
    $checks = @(
        @("PCS_PERIOD_LIST", 0, 1, "Current MTD"),
        @("PCS_LOB_LIST", 0, 2, "All"),
        @("PCS_TL_ACTIVE", 3, 4, "All", ("TL|" + $selectedLob.Replace("|", "/"))),
        @("PCS_AGENT_ACTIVE", 5, 6, "All", ("AGENT|" + $selectedLob.Replace("|", "/") + "|" + $selectedTeam.Replace("|", "/")))
    )
        ...
        foreach ($check in $checks) {
            ...
            try {
                ...
                foreach ($cell in $resolved.Cells) {
                    try {
                        if ($cell.Column -ne [int]$check[2]) {
                            throw "$($check[0]) points to the wrong selector column at row $($cell.Row)"
                        }
                        if ([int]$check[1] -gt 0) {
                            $actualGroup = [string]$filterSheet.Cells.Item($cell.Row, [int]$check[1]).Value2
                            if ($actualGroup -ne [string]$check[4]) {
                                throw "$($check[0]) crossed into '$actualGroup' instead of '$($check[4])' at row $($cell.Row)"
                            }
                        }
                        if ([string]$cell.Value2 -eq [string]$check[3]) {
                            $seenRequired = $true
                        }
                    }
                ...
                if (-not $seenRequired) {
                    throw "$($check[0]) is missing required value '$($check[3])'"
                }
            }
            finally {
...
try {
    ...
    if ($Action -eq "Install") {
        ...
        Remove-StarterTable "_PCS_FILTERS" "tblPcsFilters" 4 6
    ...
    $script:Saved = $true
    Write-Output "PCS WORKBOOK READY | $Action | $resolvedWorkbook"
}
catch {

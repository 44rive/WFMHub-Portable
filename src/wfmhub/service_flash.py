...
def _workforce_by_hour(
    ...
    for hour in range(profile.operating_start_hour, profile.operating_end_hour + 1):
        ...
        fields = (
            "scheduled_hc", "logged_hc", "productive_hc", "available_hc",
            "unavailable_hc", "bo_hc", "no_show_hc",
        )
...
def _attendance_pulse(
    ...
    for row in attendance:
        ...
        proven_no_show = bool(
            ...
            and "NO SHOW" in result and not bool(row.get("is_provisional"))
        )
        offline_now = bool(
        ...
        elif state.startswith("UNKNOWN") and not (
            call_action == "CALL_NOT_SEEN_NOW" and row_evidence != "NONE"
        ):
            call_action = "CHECK_DATA_POSSIBLE_NO_SHOW"
        reliable_action = (
            call_action in {"CALL_NO_SHOW", "CALL_NOT_SEEN_NOW", "CALL_LATE"}
        ...
        pulse.append({
            ...
            **row, "scheduled_now": scheduled_now,
            ...
            "timeline_segments": agent_segments,
        })
    due_rows = [
...
def _hourly_model(
    ...
    workforce = _workforce_by_hour(profile, report_day, attendance, pulse or {}, rulebook)
    cutoff = max(by_hour) if by_hour else None
    if pulse is not None:
        checkpoint = _as_datetime(pulse.get("checkpoint"))
        snapshot_hour = cutoff if cutoff is not None else (
            checkpoint.hour if checkpoint is not None else None
        )
        if snapshot_hour is not None:
            snapshot_hour = min(max(snapshot_hour, profile.operating_start_hour), profile.operating_end_hour)
            pulse.update({
                key: value for key, value in workforce.get(snapshot_hour, {}).items()
                if key != "no_show_hc"
            })
    ...
    if total is not None:
        ...
        # Headcounts on the day-total row are a snapshot, never summed hours.
        for field in (
            "scheduled_hc", "logged_hc", "productive_hc", "available_hc",
            "unavailable_hc", "bo_hc",
        ):
            total[field] = workforce.get(cutoff, {}).get(field)
    group_totals = {
        group.label: _aggregate(
...
def _table_value_format(book: DecisionWorkbook, header: str):
    ...
    if any(token in lowered for token in (
        ...
        "no show hc", "scheduled hc", "logged hc", "productive hc",
        "available hc", "unavailable hc", "bo hc",
    )):
...
def _flash_columns(
    ...
    if profile.flash_layout == "oem_split":
        headers = [
            ...
            "Variance", "Routed Rate", "AHT", "No Show HC",
            "Scheduled HC", "Logged HC", "Productive HC", "Available HC",
            "Unavailable HC", "BO HC", "Data State",
        ]
        rows = []
        for row in hourly:
            ...
            rows.append([
                ...
                row["aht_seconds"], row.get("no_show_hc"),
                row.get("scheduled_hc"), row.get("logged_hc"),
                row.get("productive_hc"), row.get("available_hc"),
                row.get("unavailable_hc"), row.get("bo_hc"), row["data_state"],
            ])
        return headers, rows, 1, 2, 5
    headers = [
        "Hour", "Forecast", "Actual", "Volume Handled", "Handled in SL",
        "Variance", "TSL", "Routed Rate", "AHT", "No Show HC",
        "Scheduled HC", "Logged HC", "Productive HC", "Available HC",
        "Unavailable HC", "BO HC", "Data State",
    ]
    rows = [[
        ...
        row.get("no_show_hc"), row.get("scheduled_hc"),
        row.get("logged_hc"), row.get("productive_hc"),
        row.get("available_hc"), row.get("unavailable_hc"),
        row.get("bo_hc"), row["data_state"],
    ] for row in hourly]
...
def _add_flash_sheet(
    ...
    if profile.flash_layout == "oem_split":
        ...
        values = [
            ...
            total_values.get("scheduled_hc"), total_values.get("logged_hc"),
            total_values.get("productive_hc"), total_values.get("available_hc"),
            total_values.get("unavailable_hc"), total_values.get("bo_hc"),
...
def _add_control_sheet(
    ...
    headers = [
        ...
        "Scheduled HC", "Logged HC", "Productive HC", "Available HC",
        "Unavailable HC", "BO HC", "PTO / Away HC", "Offline Now",
        "Possible No Show HC", "Call Now", "Status",
    ]
    table_row = 34
    ...
    for offset, (profile, total, cutoff, pulse) in enumerate(summaries, table_row + 1):
        ...
        row = [
            ...
            pulse.get("no_show_hc"), value.get("scheduled_hc"),
            value.get("logged_hc"), value.get("productive_hc"),
            value.get("available_hc"), value.get("unavailable_hc"),
            value.get("bo_hc"), pulse.get("time_off_hc"),
            pulse.get("offline_now"), pulse.get("unknown_hc"),
...
def _issues_and_drivers_rows(
    ...
    pulse_by_profile: dict[str, Sequence[dict[str, Any]]] | None = None,
    rulebook: Rulebook | None = None,
) -> tuple[list[str], list[tuple[Any, ...]]]:
    """Return below-target queue-hours with relevant agent-state context."""

    headers = [
        "item_type", "lob", "hour_or_queue", "volume", "tsl", "target",
        "gap_to_target", "late_routed", "lost_contacts",
        "issue_or_driver", "action", "impacted_hour", "agent_names",
    ]
    ...
    for profile in profiles:
        ...
        for row in hourly_by_profile.get(profile.profile_id, ()):
            if row.get("data_state") == "FORECAST MISSING":
                output.append((
                    ...
                    "FORECAST MISSING", "Load or map the forecast extract", None, None,
                ))
            elif (
                ...
                output.append((
                    ...
                    "NO MAPPED CALLS", "Confirm zero demand or review source mapping", None, None,
                ))
        ...
        if not pulse.get("agent_rows"):
            output.append((
                ...
                "NO SCHEDULED AGENT ROWS", "Check schedule and active FTE coverage", None, None,
            ))
            ...
            output.append((
                ...
                "Validate Agent Status evidence before calling", None, None,
            ))
        ...
        by_queue: dict[tuple[str, int], list[dict[str, Any]]] = defaultdict(list)
        for row in day_rows:
            stamp = _as_datetime(row.get("hour_start"))
            if stamp is not None:
                by_queue[(str(row.get("queue") or "").strip().upper(), stamp.hour)].append(row)
        configured = (
            list(profile.flash_queues)
            if profile.flash_queues else sorted({queue for queue, _hour in by_queue})
        )
        ...
        for queue in configured:
            for hour in range(24):
                rows = by_queue.get((str(queue).strip().upper(), hour), [])
                ...
                if service_level is not None and (target is None or service_level >= target):
                    continue
                late_connected = max(0.0, aggregate["answered"] - aggregate["answered_within_target"])
                lost_in_denominator = max(0.0, aggregate["abandoned"] - aggregate["abandoned_within_target"])
                if service_level is None:
                    diagnosis, action = "NO SLA SAMPLE", "Review queue counters and source coverage"
                elif late_connected > lost_in_denominator * 1.5:
                    diagnosis, action = "LATE ROUTED CONTACTS", "Review answer delay and staffing"
                elif lost_in_denominator > late_connected * 1.5:
                    diagnosis, action = "LOST CONTACTS", "Review abandonment and coverage"
                else:
                    diagnosis, action = "MIXED DELAY AND LOSS", "Review demand, delay and coverage"
                impacted_start = datetime.combine(report_day, datetime.min.time()) + timedelta(hours=hour)
                impacted_end = impacted_start + timedelta(hours=1)
                names = []
                for agent in (pulse_by_profile or {}).get(profile.profile_id, ()):
                    if not any(start < impacted_end and end > impacted_start for start, end in agent.get("expected_work_intervals", ())):
                        continue
                    if agent.get("no_show"):
                        names.append(f"{agent.get('agent_name') or agent.get('agent_id')} [No show]")
                        continue
                    for segment in agent.get("timeline_segments", ()):
                        left, right = _as_datetime(segment.get("segment_start")), _as_datetime(segment.get("segment_end"))
                        if left is None or right is None or left >= impacted_end or right <= impacted_start:
                            continue
                        category = str(segment.get("actual_category") or "").casefold()
                        status_rule = rulebook.classify_status(segment.get("actual_status")) if rulebook else None
                        aux = str(status_rule.aux_classification if status_rule else "").casefold()
                        capacity_reducing = (
                            bool(segment.get("is_gap"))
                            or category in {"auxiliary", "break", "lunch", "unavailable"}
                            or aux == "bo"
                        )
                        if capacity_reducing:
                            names.append(f"{agent.get('agent_name') or agent.get('agent_id')} [{segment.get('actual_status') or segment.get('mismatch_type') or category}]")
                            break
                        if category == "no_status_evidence":
                            names.append(f"{agent.get('agent_name') or agent.get('agent_id')} [Evidence missing]")
                            break
                output.append((
                    "QUEUE DRIVER", profile.label, queue, aggregate.get("offered"),
                    service_level, target,
                    service_level - target if service_level is not None and target is not None else None,
                    late_connected, lost_in_denominator, diagnosis, action,
                    impacted_start, "; ".join(sorted(set(names))) or "No observed agent gap",
                ))
...
def build_service_flashes_workbook(
    ...
    try:
        ...
        issue_headers, issue_rows = _issues_and_drivers_rows(
            ...
            cutoff_by_profile, pulse_summary_by_profile, pulse_by_profile,
            rulebook,
        )
        ...
        book.definitions([
            ...
            ("Routed Rate", "Total routed / total entered", "Service availability", "Storm business reference; not agent availability or adherence"),
            ...
            ("No Show HC", "Completed scheduled shifts with no presence and explicit agent-specific evidence", "Confirmed no-show population", "In-progress not-seen is Possible No Show, not confirmed No Show"),
            ("Scheduled HC", "Distinct agents expected to work at the hourly snapshot after PTO/Away", "Staffing context", "Snapshot is hour-end or current evidence checkpoint; day total uses latest actual hour"),
            ("Logged HC", "Distinct scheduled agents with Agent Status or LILO presence at the snapshot", "Observed staffing", "Missing status evidence is not assumed logged"),
            ("Productive HC", "Distinct scheduled agents in a Productive Agent Status category", "Available, inbound, outbound, and BO included", "Available HC and BO HC are subsets, not additive to Productive HC"),
            ("Available HC", "Productive agents classified Available by the effective status rule", "Ready-to-handle snapshot", "Requires an Agent Status label; LILO-only is not assigned an AUX"),
            ("Unavailable HC", "Agents in Auxiliary, Break, Lunch, or Unavailable categories", "Other-AUX snapshot", "Excludes Logged Off; BO is classified Productive"),
            ("BO HC", "Productive agents whose AUX classification is BO", "Back-office snapshot", "Subset of Productive HC"),
            ...
            ("Issues & Drivers", "Missing source evidence plus below-target configured queue-hours with real demand", "One operational exception list", "Agent names show same-LOB offline, AUX, BO or unknown-state context, not individual causality"),
        ])
        ...
        book.audit([
            ...
            ("Design reference", "WFMHub report design", "Four business Flash scopes rendered as native Excel"),

...
def _existing_actions(config: Config) -> list[tuple[Any, ...]]:
    """Preserve keyed coaching rows from the current collaborative tracker."""

    selected: dict[str, dict[str, Any]] = {}
    for row in _read_actions_from(tracker_path(config)):
...
def _add_filter_names(workbook) -> None:
    ...
    workbook.define_name("PCS_LOB_LIST", list_range("B"))
    team_group = '"TL|"&SUBSTITUTE(OVERVIEW!$H$3,"|","/")'
    workbook.define_name(
        "PCS_TL_ACTIVE",
        group_range(team_group, "C", "D"),
    )
    agent_group = (
...
def _add_overview(
    ...
    agent_capacity = max(50, min(1000, sum(
        1 for row in filter_rows if row[4] == "AGENT|All|All"
    ) - 1))
...
def ensure_pcs_tracker(
    ...
    *,
    ...
    try:
        ...
        _add_coaching(report, coaching_rows, existing_actions)
        ...
        raise RuntimeError(
            ...
            "The tracker rebuild requires an exclusive file lock; existing "
            "coaching rows were read before replacement was attempted."
        ) from exc
    except Exception:
        partial.unlink(missing_ok=True)
        raise

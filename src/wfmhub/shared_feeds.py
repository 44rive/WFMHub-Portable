...
workbooks are not: team comments and action logs are never rewritten here.
"""
...
PCS_FEED_SCHEMA_VERSION = "7"
PCS_FILTER_HEADERS = (
    "Period", "LOB", "Team Key", "Team Leader", "Agent Key", "Agent",
)
...
def pcs_filter_rows(
    ...
    periods = tuple(window.label for window in windows)
    lob_values = ("All", *lobs)
    team_rows: list[tuple[str, str]] = []
    agent_rows: list[tuple[str, str]] = []
    for lob in ("All", *lobs):
        team_rows.extend((f"TL|{_key_part(lob)}", value) for value in ("All", *leaders[lob]))
        for leader in ("All", *leaders[lob]):
            agent_rows.extend(
                (f"AGENT|{_key_part(lob)}|{_key_part(leader)}", value)
                for value in ("All", *agents[(lob, leader)])
            )
    return [
        (
            periods[index] if index < len(periods) else "",
            lob_values[index] if index < len(lob_values) else "",
            *(team_rows[index] if index < len(team_rows) else ("", "")),
            *(agent_rows[index] if index < len(agent_rows) else ("", "")),
        )
        for index in range(max(len(periods), len(lob_values), len(team_rows), len(agent_rows)))
    ]

...
from .report_packs import REPORT_PACK_KEYS, build_report_pack
...
from .governance_catalog import build_kpi_catalog
...
SOURCE_GROUPS = {
    "all": None,
    "attendance": {"fte", "schedule", "lilo", "agent_status"},
    "rtm": {"fte", "schedule", "lilo", "agent_status", "calls", "forecast"},
    "pcs": {"fte", "calls"},
}
...
def setup(home: Path, source_root: Path | None, non_interactive: bool) -> int:
    ...
    print(f"Clean feed  : {config.feed}")
...
def build_reports(
    ...
    try:
        with write_session(config) as conn:
            pcs_only = bool(packs) and set(packs) <= {"pcs"}
            model = (
                refresh_pcs_models(
...
def rules_tool(
    ...
    for lines in (
        ...
        validate_report_catalog(reports, REPORT_PACK_KEYS),
...
def _choose_source_group() -> str:
    ...
    try:
        return {"1": "all", "2": "attendance", "3": "rtm", "4": "pcs"}[choice]
...
def _build_latest_pcs_report(home: Path) -> None:
    ...
    if result:
        raise RuntimeError(
            "The report was not approved because one or more source files were rejected. "
...
def menu(home: Path) -> int:
    while True:
        ...
        print("    [4] PCS Report & Coaching")
        print("    [5] Realisations")
        print("    [6] Bonus")
        print("\n  ANALYZE")
        print("    [7] Analyze a period")
        print("    [8] Export clean data")
        print("    [9] Open local WFM Manager Workbench")
        print("\n  IN DEVELOPMENT")
        print("   [10] Staffing")
        print("   [11] Final Absenteeism")
        print("\n  SETTINGS")
...
def parser() -> argparse.ArgumentParser:
    ...
    refresh_p.add_argument("--pack", action="append", choices=REPORT_PACK_KEYS)
    ...
    report_p.add_argument("--pack", choices=REPORT_PACK_KEYS, default="pcs")
    ...
    web_p = commands.add_parser("web", help="Open the localhost WFM Manager Workbench")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "refresh":
            packs = () if args.no_report else REPORT_PACK_KEYS if args.all_packs else tuple(args.pack or ["service"])

        ...
        try:
            ...
            controls = _rows(conn.execute(
                """WITH status_totals AS (
                       SELECT agent_day_key,
                              sum(CASE WHEN actual_category='Break'
                                       AND observed_source='AGENT_STATUS'
                                       THEN segment_minutes ELSE 0 END) AS break_minutes,
                              sum(CASE WHEN actual_category='Lunch'
                                       AND observed_source='AGENT_STATUS'
                                       THEN segment_minutes ELSE 0 END) AS lunch_minutes,
                              sum(CASE WHEN observed_source='AGENT_STATUS'
                                       THEN segment_minutes ELSE 0 END) AS covered_minutes
                       FROM mart.shift_timeline_segment
                       GROUP BY agent_day_key
                   )
                   SELECT a.business_date, a.agent_id, a.agent_name,
                          a.team_leader, a.lob, a.scheduled_start, a.scheduled_end,
                          CASE WHEN a.scheduled_minutes>0
                               THEN 1.0*coalesce(s.covered_minutes,0)/a.scheduled_minutes
                               END AS status_coverage_percent,
                          coalesce(s.break_minutes,0) AS break_minutes,
                          max(0,coalesce(s.break_minutes,0)-?) AS break_overrun_minutes,
                          coalesce(s.lunch_minutes,0) AS lunch_minutes,
                          max(0,coalesce(s.lunch_minutes,0)-?) AS lunch_overrun_minutes,
                          'AGENT_STATUS_TIMELINE' AS measurement_basis
                   FROM mart.attendance_agent_day a
                   LEFT JOIN status_totals s ON s.agent_day_key=a.agent_day_key
                   WHERE a.business_date BETWEEN ? AND ?
                     AND a.shift_state='COMPLETE'
                   ORDER BY a.business_date DESC,
                            (max(0,coalesce(s.break_minutes,0)-?)
                             +max(0,coalesce(s.lunch_minutes,0)-?)) DESC,
                            a.agent_name""",
                (
                    self.config.rules.break_minutes,
                    self.config.rules.lunch_minutes,
                    scope.start, scope.end,
                    self.config.rules.break_minutes,
                    self.config.rules.lunch_minutes,
                ),

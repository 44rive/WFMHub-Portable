...
Then copy all six current PCS CSVs from the Hub's displayed folder to the fixed
SharePoint `Source CSV` folder. Wait for OneDrive sync; open the **same** shared
workbook in desktop Excel, choose **Data > Refresh All**, wait, and save.
Coaching owners edit only the coaching action table. See [PCS.md](PCS.md) and
the [one-time Power Query guide](PCS_POWER_QUERY_SETUP.md). Set the dedicated
PCS path to the existing SharePoint workbook after moving it there. Keep SQLite
and other generated reports local.
